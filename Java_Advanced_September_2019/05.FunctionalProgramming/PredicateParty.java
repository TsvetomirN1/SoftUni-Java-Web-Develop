...
public class PredicateParty {
    ...
    private static final BiPredicate<String, String> removeStartsWithPredicate
            = (name, start) -> !name.startsWith(start);

    private static final BiPredicate<String, String> doubleStartsWithPredicate
            = String::startsWith;

    private static final BiPredicate<String, String> removeEndsWithPredicate =
            (name, end) -> !name.endsWith(end);

    private static final BiPredicate<String, String> doubleEndsWithPredicate =
            String::endsWith;

    private static final BiPredicate<String, Integer> removeLengthPredicate =
            (name, length) -> name.length() != length;

    private static final BiPredicate<String, Integer> doubleLengthPredicate =
            (name, length) -> name.length() == length;
    ...
    private static List<String> constraint(String[] commands, List<String> collection) {
        if (commands[0].equals("StartsWith")) {
            ...
            for (int i = 0; i < guestsMatchingCriteria.size(); i++) {
                collection.add(collection.indexOf(guestsMatchingCriteria.get(i)),
                        guestsMatchingCriteria.get(i));
            }
        } else if (commands[0].equals("EndsWith")) {
            String end = commands[1];
            List<String> guestsMatchingCriteria = collection.stream()
